...
import 'package:test_telemedicina/repository/use_cases/dermatologist.dart';

import '../constants.dart';

class Dermatology extends SpecialistsCommonDatasource {
  @override
  Future<void> updateData({bool delay = false}) async {
    try {
      ...
      final List<Dermatologist> _specialistsList =
          DermatologistModel.fromList(_json);
  ...
  void setErrorStatus(String error, {int status = 500}) {
    super.specialistData.clear();
    super.specialistData.add(DermatologistError(error, statusCode: status));
  }
}
